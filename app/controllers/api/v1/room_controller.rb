class Api::V1::RoomController < Api::V1::ApplicationController
  ...
  def listIdol
    ...
    if params[:category_id].nil?
      sql = "select * from (SELECT rooms.*, schedules.room_id, schedules.start FROM rooms LEFT JOIN schedules ON rooms.id = schedules.room_id LEFT JOIN broadcasters ON rooms.broadcaster_id = broadcasters.id WHERE broadcasters.deleted = false and rooms.is_privated = false and on_air = 1 ORDER BY schedules.start ASC) as schedule GROUP BY id ORDER BY on_air desc limit #{limit} offset #{offset}"
