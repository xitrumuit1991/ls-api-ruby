class Api::V1::RanksController < Api::V1::ApplicationController
	...
	def topMonth
		...
		hearts = HeartLog.select('room_id, sum(quantity) as quantity').where(created_at: 1.month.ago.beginning_of_day..DateTime.now).group(:room_id).order('quantity DESC').limit(5)
		hearts.each do |heart|
